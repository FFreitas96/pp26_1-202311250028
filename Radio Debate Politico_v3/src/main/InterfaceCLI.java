...
import state.EstadoAguardandoCiclo;
import state.EstadoEmDireitoDeResposta;
...
public class InterfaceCLI {
    ...
    public void executar() {
        ...
        while (rodando) {
            ...
            switch (op) {
                ...
                case "9":  solicitarDR();          break;
                case "10": concederDR();           break;
                case "11": negarDR();              break;
                case "12": finalizarDebate();      break;
    ...
    private void cabecalho() {
        ...
        System.out.println("  Builder | Prototype | Observer | Mediator | Facade | State");
        System.out.println("================================================================\n");
    }

    private void menu() {
        String estado = fachada.getGerenciador().getEstadoAtual().getClass().getSimpleName();
        int    filaDR = fachada.getGerenciador().getFilaDRSize();
        ...
        System.out.println("9  - Solicitar Direito de Resposta  (State)");
        System.out.println("10 - Conceder DR               (State) [fila: " + filaDR + "]");
        System.out.println("11 - Negar DR                  (State) [fila: " + filaDR + "]");
        System.out.println("12 - Finalizar debate e gerar relatorio");
    ...
    private String faseLabel() {
        if (fachada.getGerenciador().getEstadoAtual() instanceof EstadoEmDireitoDeResposta) {
            return "EM DIREITO DE RESPOSTA";
        }
        if (fachada.getGerenciador().getEstadoAtual() instanceof EstadoAguardandoCiclo) {
            return "aguardando decisao DR";
        }
        return faseIndice >= fases.length ? "rodada encerrada" : fases[faseIndice];
    }

    // ==================== Métodos existentes (v2.0) ====================
    ...
    private void iniciarProximaFase() {
        ...
        // Se estiver em DR, avanca a fila de defesas
        if (fachada.getGerenciador().getEstadoAtual() instanceof EstadoEmDireitoDeResposta) {
            System.out.println("  [State] Avancando defesa de DR...");
            fachada.avancarEtapa();
            return;
        }

        // Se aguardando decisao de DR, informar o gerente
        if (fachada.getGerenciador().getEstadoAtual() instanceof EstadoAguardandoCiclo) {
            System.out.println("  [State] Ha Direito(s) de Resposta pendente(s).");
            System.out.println("  Use opcao 10 para CONCEDER ou opcao 11 para NEGAR o DR.");
            return;
        }
        ...
        if (faseIndice >= fases.length) {
            // Verificar se ha DR pendentes apos a TREPLICA
            if (fachada.getGerenciador().getEstadoAtual() instanceof EstadoAguardandoCiclo) {
                System.out.println("\n  >> Rodada concluida! Ha Direito(s) de Resposta pendente(s).");
                System.out.println("  >> Use opcao 10 (conceder) ou 11 (negar) para prosseguir.");
            } else {
                System.out.println("\n  >> Rodada concluida!");
            }
        }
    ...
    // ==================== Métodos novos — State (v3.0) ====================

    private void solicitarDR() {
        if (!candidatosCadastrados) { System.out.println("Cadastre candidatos primeiro."); return; }

        if (fachada.getGerenciador().getEstadoAtual() instanceof EstadoEmDireitoDeResposta) {
            System.out.println("  [State] Solicitacoes de DR bloqueadas durante as defesas.");
            return;
        }

        System.out.println("[State] Candidatos que podem solicitar DR:");
        for (Candidato c : fachada.getGerenciador().getCandidatos()) {
            String status = c.isSolicitouDR() ? " [ja solicitou]" : "";
            System.out.println("  [" + c.getId() + "] " + c.getNome()
                    + " (" + c.getPartido() + ")" + status);
        }
        System.out.print("ID do candidato solicitante: ");
        try {
            int id = Integer.parseInt(scanner.nextLine().trim());
            fachada.solicitarDR(id);
        } catch (NumberFormatException e) { System.out.println("ID invalido."); }
    }

    private void concederDR() {
        if (!(fachada.getGerenciador().getEstadoAtual() instanceof EstadoAguardandoCiclo)) {
            System.out.println("  [State] Nenhum DR pendente para conceder no momento.");
            return;
        }
        fachada.concederDR();
    }
    ...
    private void negarDR() {
        if (!(fachada.getGerenciador().getEstadoAtual() instanceof EstadoAguardandoCiclo)) {
            System.out.println("  [State] Nenhum DR pendente para negar no momento.");
            return;
        }
        fachada.negarDR();
    }
